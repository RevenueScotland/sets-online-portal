...
class DeleteAttachmentFilesJob < ApplicationJob
  ...
  def self.schedule_next_run(how_long = nil)
    ...
    super(get_interval(how_long, Rails.configuration.x.scheduled_jobs.delete_attachment_files_job_run_every,
  ...
  def job_action
    days = Rails.configuration.x.scheduled_jobs.delete_attachment_files_days_old
    FileStorageHelper.delete_stored_files(days)
  end
end
