...
# Job to refresh the SystemParameters cache.
...
class RefreshSystemParametersJob < ApplicationJob
  ...
  def self.schedule_next_run(how_long = nil)
    ...
    super(get_interval(how_long, Rails.configuration.x.scheduled_jobs.refresh_sys_params_every, FAILSAFE_SCHEDULE_TIME))
  end

  private

  # This method is what the job is set up to actually do.
  # Calls the {ReferenceData::SystemParameter.refresh_cache!} to update cache
  def job_action
