...
module Returns
  ...
  class AbstractReturnValidator < ActiveModel::Validator
    ...
    # @param blamed [String] details about the attribute/section on which errors occured ie the area that's got errors
    # @param model [ActiveModel] object containing errors to be logged
    def add_error(error_list, blamed, model)
      # always log something (ie even if errors is blank)
      Rails.logger.info "Errors validating: #{blamed}"
