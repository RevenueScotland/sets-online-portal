...
module ReferenceData
  ...
  class CodeXrefValue < ReferenceDataCaching
    # additional fields
    attr_accessor :code, :value, :description
    ...
    private_class_method def self.make_object(data)
      ...
      CodeXrefValue.new(domain_code: data[:cxd_domain], service_code: data[:cxd_srv_code],
                        workplace_code: data[:workplace_code],
                        code: data[:system_value], value: data[:site_value], description: data[:description])
    end
    ...
    private_class_method def self.back_office_data
      lookup_back_office_data(:get_code_xref_values, :reference_values)
    end
