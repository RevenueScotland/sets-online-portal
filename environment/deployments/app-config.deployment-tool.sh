...
if [[ -n "$version" ]] && [[ -n "$environment" ]]  && [[ -n "$dockerImageName" ]]; then
	...
	echo export INT_PROXY_HTTP_PORT=4400 >> scratch/$version/$environment/additional.env
	echo export INT_PROXY_HTTPS_PORT=4401 >> scratch/$version/$environment/additional.env
	echo export PROXY_HTTP_PORT=4400 >> scratch/$version/$environment/additional.env
	echo export PROXY_HTTPS_PORT=4401 >> scratch/$version/$environment/additional.env
	...
	echo FL_ENDPOINT_ROOT='http://ps-axway-gw.global.internal:33080/communication' >> scratch/$version/$environment/password.env
