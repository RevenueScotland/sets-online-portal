...
if [[ -n "$version" ]] && [[ -n "$environment" ]]  && [[ -n "$dockerImageName" ]]; then
	...
	echo export INT_PROXY_HTTP_PORT=4200 >> scratch/$version/$environment/additional.env
	echo export INT_PROXY_HTTPS_PORT=4201 >> scratch/$version/$environment/additional.env
	echo export PROXY_HTTP_PORT=4200 >> scratch/$version/$environment/additional.env
	echo export PROXY_HTTPS_PORT=4201 >> scratch/$version/$environment/additional.env
	...
	echo FL_ENDPOINT_ROOT='http://ps-axway-gw.global.internal:22080/communication' >> scratch/$version/$environment/password.env
