...
if [[ -n "$version" ]] && [[ -n "$environment" ]]  && [[ -n "$dockerImageName" ]]; then
	...
	echo export INT_PROXY_HTTP_PORT=4010 >> scratch/$version/$environment/additional.env
	echo export INT_PROXY_HTTPS_PORT=4011 >> scratch/$version/$environment/additional.env
	echo export PROXY_HTTP_PORT=4010 >> scratch/$version/$environment/additional.env
	echo export PROXY_HTTPS_PORT=4011 >> scratch/$version/$environment/additional.env
	...
	echo FL_ENDPOINT_ROOT='http://lg-axway-qa.development.local:37080/communication' >> scratch/$version/$environment/password.env
