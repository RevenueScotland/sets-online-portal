...
if [[ -n "$version" ]] && [[ -n "$environment" ]]  && [[ -n "$dockerImageName" ]]; then
	...
	echo export INT_PROXY_HTTP_PORT=4020 >> scratch/$version/$environment/additional.env
	echo export INT_PROXY_HTTPS_PORT=4021 >> scratch/$version/$environment/additional.env
	echo export PROXY_HTTP_PORT=4020 >> scratch/$version/$environment/additional.env
	echo export PROXY_HTTPS_PORT=4021 >> scratch/$version/$environment/additional.env
	...
	echo FL_ENDPOINT_ROOT='http://lg-axway-qa.development.local:28080/communication' >> scratch/$version/$environment/password.env
