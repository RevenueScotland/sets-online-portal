...
if [[ -n "$version" ]] && [[ -n "$environment" ]]  && [[ -n "$dockerImageName" ]]; then
	...
	echo export PROXY_HTTP_PORT=80 >> scratch/$version/$environment/additional.env
	echo export PROXY_HTTPS_PORT=443 >> scratch/$version/$environment/additional.env
	...
	echo FL_ENDPOINT_ROOT='http://lg-axway-qa.development.local:35080/communication' >> scratch/$version/$environment/password.env
