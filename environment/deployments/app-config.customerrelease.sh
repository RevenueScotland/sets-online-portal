...
# This is used by the deployment - tool to replace values. Any specific settings
# found in this file can be overridden by values in test-environments.yml file.
...
if [[ -n "$version" ]] && [[ -n "$environment" ]]  && [[ -n "$dockerImageName" ]]; then
	...
	echo export INT_PROXY_HTTP_PORT=4100 >> scratch/$version/$environment/additional.env
	echo export INT_PROXY_HTTPS_PORT=4101 >> scratch/$version/$environment/additional.env
	echo export PROXY_HTTP_PORT=4100 >> scratch/$version/$environment/additional.env
	echo export PROXY_HTTPS_PORT=4101 >> scratch/$version/$environment/additional.env
	...
	echo FL_ENDPOINT_ROOT='http://lg-axway-qa.development.local:22080/communication' >> scratch/$version/$environment/password.env
