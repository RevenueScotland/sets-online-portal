...
class RefreshSystemParametersJobTest < ActiveJob::TestCase
  ...
  class TestJob < RefreshSystemParametersJob
    private
  ...
  test 'schedule_next_run' do
    ...
    assert_equal(Rails.configuration.x.scheduled_jobs.refresh_sys_params_every, how_long)
    assert_enqueued_jobs(1, only: TestJob)

    schedule_test(50, 50, 2)
    schedule_test(0, Rails.configuration.x.scheduled_jobs.refresh_sys_params_every, 3)
    schedule_test(nil, Rails.configuration.x.scheduled_jobs.refresh_sys_params_every, 4)
  end
