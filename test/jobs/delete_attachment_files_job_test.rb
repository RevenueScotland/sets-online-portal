...
class DeleteAttachmentFilesJobTest < ActiveJob::TestCase
  # Overwrite raise an exception
  class TestExceptionJob < DeleteAttachmentFilesJob
    private
  ...
  # test job
  class TestJob < DeleteAttachmentFilesJob
    private
  ...
  test 'schedule_next_run' do
    ...
    assert_equal(Rails.configuration.x.scheduled_jobs.delete_attachment_files_job_run_every, how_long)
    assert_enqueued_jobs(1, only: TestJob)

    schedule_test(50, 50, 2)
    schedule_test(0, Rails.configuration.x.scheduled_jobs.delete_attachment_files_job_run_every, 3)
    schedule_test(nil, Rails.configuration.x.scheduled_jobs.delete_attachment_files_job_run_every, 4)
  end
